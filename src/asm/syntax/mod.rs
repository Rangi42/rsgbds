    ...
    loop {
        while !parse_ctx.lexer.top_context_mut().is_empty() {
            ...
            if !line_tokens.is_empty() {
                match parser::LineParser::new().parse(&mut parse_ctx, line_tokens.drain(..)) {
                    Ok(()) => {} // Continue parsing.

                    // This is generated by the built-in lexer, but we don't use it.
                    Err(ParseError::InvalidToken { .. }) => unreachable!(),
                    Err(ParseError::UnrecognizedEof { location, expected }) => {
                        let span = &parse_ctx.line_spans[location];
                    ...
                    Err(ParseError::UnrecognizedToken {
                        ...
                        let span = &parse_ctx.line_spans[span_idx];
                        parse_ctx.error(span, |error| {
                    ...
                    Err(ParseError::User { error: () }) => break,
                }
            }
        }
