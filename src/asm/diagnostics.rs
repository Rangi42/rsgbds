...
pub fn error<'span, F: FnOnce(&mut ReportBuilder<'span>)>(
    ...
    build(&mut error);
    add_backtrace_labels_and_print(error, span, options.backtrace_depth);
...
pub fn warn<'span, F: FnOnce(&mut ReportBuilder<'span>)>(
    ...
    let mut warning = Report::build(kind, span)
        ...
        .with_code(id);
    build(&mut warning);
    add_backtrace_labels_and_print(warning, span, options.backtrace_depth);
    if kind == ERROR_KIND {
        decrement_error_count(nb_errors_left, options);
    }
}

pub fn warn_special<'span, F: FnOnce(&mut ReportBuilder<'span>)>(
    span: &'span Span,
    backtrace_depth: usize,
    build: F,
) {
    let mut warning = Report::build(WARNING_KIND, span)
        .with_config(Config::default().with_index_type(IndexType::Byte));
    build(&mut warning);
    add_backtrace_labels_and_print(warning, span, backtrace_depth);
}
...
fn add_backtrace_labels_and_print(diag: ReportBuilder<'_>, span: &Span, backtrace_depth: usize) {
    ...
    add_level(diag, span, backtrace_depth);
}
