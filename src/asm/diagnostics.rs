...
use crate::{
    context_stack::{ContextStack, SourceContext, SourceKind, SourceNode, SourcesMut, Span},
...
pub fn error<F: FnOnce(&mut ReportBuilder)>(
    ...
    build(&mut error);
    add_backtrace_labels(&mut error, span);
    error
        .finish()
        .eprint(sources)
...
pub fn lex_error<F: FnOnce(ReportBuilder, RawSpan) -> ReportBuilder>(
    ...
    // TODO: figure out a way to generate a backtrace...
    ...
    .eprint(source_store)
    .expect("Failed to print diagnostic");
    decrement_error_count(source_store, nb_errors_left, options);
}
...
pub fn warn<F: FnOnce(&mut ReportBuilder)>(
    ...
    let raw_span = span.resolve();
    let mut warning = Report::build(kind, raw_span)
        .with_config(Config::default().with_index_type(IndexType::Byte))
        .with_code(id);
    build(&mut warning);
    add_backtrace_labels(&mut warning, span);
    warning
        .finish()
        .eprint(sources)
        .expect("Failed to print diagnostic");
...
fn add_backtrace_labels(diag: &mut ReportBuilder, span: &Span<'_>) {
    let Some(source) = span.src.as_ref() else {
        return;
    };
    let guard = source.get();
    let sources = guard.sources();
    let mut node = guard.deref();

    while let Some((node_id, byte_span)) = node.parent.as_ref() {
        let parent_node = &sources[*node_id];
        let msg = match &node.kind {
            SourceKind::File(_) => "File included from here",
            SourceKind::Macro { .. } => "Macro called here",
            SourceKind::Rept { .. } => "Loop beginning here",
        };
        diag.add_label(note_label(parent_node.resolve(byte_span)).with_message(msg));
        node = parent_node;
    }
}

fn add_backtrace_labels_raw(diag: &mut ReportBuilder) {}
